...
export default function GitBranchStatusModal({
  ...
  const handleCheckoutToMain = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to checkout to main repository')
    } finally {
  ...
  const handleCreateBranch = useCallback(async () => {
    ...
    try {
      await apiClient.createTaskBranch(taskId)
      onStatusUpdate?.()
      onClose()
    } catch (err) {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          {gitStatus.branch_exists ? (
            /* Commits ahead/behind — only shown when branch exists */
            <div className="flex justify-between items-center">
            ...
            <div className="flex items-start p-3 rounded bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300">
              ...
              <div>
                ...
                <p className="text-xs mt-0.5">
                  Branch '{gitStatus.branch_name}' does not exist. It may have been deleted or is not available on this machine.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="border-t border-gray-200 dark:border-white/[0.08] pt-4 space-y-3">
            ...
            {gitStatus.base_has_conflicting_uncommitted && (
              <div className="flex items-start p-3 rounded bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300">
                ...
                <span className="text-sm">Main repo has uncommitted changes in files modified by this task — commit or stash them before merging</span>
              </div>
            )}
        ...
        <div className="flex justify-end pt-2">
          <Button
            variant="ghost"
            onClick={onClose}
            disabled={checkoutLoading || abortLoading || createBranchLoading}
          >
            Close
          </Button>
        </div>
