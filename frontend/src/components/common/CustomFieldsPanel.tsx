  ...
  return (
    <div className="relative" ref={popoverRef}>
      ...
      {isOpen && allFieldNames.size > 0 && (
        <div className={`absolute right-0 top-full mt-1 z-20 w-72 ${surfaces.raised} border ${borderColors.default} rounded-lg shadow-lg`}>
          ...
          <div className="p-2 space-y-1 max-h-64 overflow-y-auto">
            {Array.from(allFieldNames).map(fieldName => {
              ...
              return (
                <div key={fieldName} className={`flex items-center justify-between px-2 py-1.5 rounded ${hoverColors.subtle}`}>
                  ...
                  <div className="flex items-center min-w-0">
                    {isOrphaned ? (
                      ...
                      <select
                        ...
                        disabled={saving || savingField === fieldName}
                        className={`text-xs px-1 py-0.5 border ${borderColors.input} rounded bg-white dark:bg-white/[0.06] ${textColors.primary} max-w-[140px] truncate`}
                      >
                        <option value="">—</option>
                        {def.options?.map(opt => (
                      ...
                      <input
                        ...
                        className={`text-xs px-1 py-0.5 border border-blue-400 rounded bg-white dark:bg-white/[0.06] ${textColors.primary} min-w-0 w-full max-w-[140px]`}
                      />
                    ) : (
                      <button
