...
#### BaseAgent Interface
All agents implement a unified interface with two core execution methods:
...
Both methods accept either a user message string or tool approval results, and generate ConversationEvent objects (messages, tool calls, tool results). The streaming interface enables real-time UI updates as the agent processes requests, providing immediate feedback to users.
...
- **Real-Time Streaming**: Events are streamed to the frontend as they occur, enabling immediate UI updates without waiting for agent completion
...
- **Unified API**: Dual endpoint design for flexible consumption:
  - `/conversations/{id}/messages` - Returns complete event list
  - `/conversations/{id}/messages/stream` - Streams events as NDJSON (newline-delimited JSON)
  - `/conversations/{id}/approve-tools/stream` - Streams tool approval responses
...
- **Progress Tracking**: Real-time visibility into agent operations as events stream in incrementally
- **Immediate Feedback**: No waiting for agent completion - see tool calls, results, and messages as they happen
- **Contextual Understanding**: Tool interactions provide context for agent responses
- **Responsive UI**: Events appear progressively, improving perceived performance
...
- **Real-Time Streaming**: Events appear progressively as they're generated by the agent
  ...
  - Agent events stream in one-by-one as they occur (tool calls, results, messages)
