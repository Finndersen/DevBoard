...
class CodebaseInvestigationService:
    ...
    def read_architecture_content(self, codebase_path: str) -> str | None:
        ...
        with logfire.span("codebase_investigation.read_architecture_content", codebase_path=codebase_path):
            try:
                ...
                logfire.info("Architecture content read successfully",
        ...
        with logfire.span("codebase_investigation.get_architecture_document", codebase_path=codebase_path):
            try:
                ...
                if arch_file.exists() and arch_file.is_file():
                    ...
                    logfire.info("Architecture document found",
                               path=str(arch_file),
                               size_bytes=size,
    ...
    def update_architecture_content(
        ...
        with logfire.span("codebase_investigation.update_architecture_content",
            ...
            try:
                ...
                if original_hash is not None:
                    if arch_file.exists():
                        ...
                        if current_hash != original_hash:
                            logfire.warn("Architecture document conflict detected",
                ...
                logfire.info("Architecture document updated successfully",
    ...
    async def generate_architecture_document(
        ...
        with logfire.span("codebase_investigation.generate_architecture_document",
                         codebase_path=codebase_path, codebase_name=codebase_name):
            try:
                codebase_dir = Path(codebase_path).resolve()
                if not codebase_dir.exists() or not codebase_dir.is_dir():
                    error_msg = f"Codebase path does not exist or is not a directory: {codebase_path}"
                ...
                logfire.info("Starting architecture generation with AI",
                           codebase=codebase_name, working_dir=str(codebase_dir))
                ...
                logfire.info("Architecture document generated successfully",
                           file_path=str(arch_file), content_length=len(ai_content))
...
ARCHITECTURE_DOCUMENT_FORMAT="""
```markdown
# <CODEBASE NAME> - Architecture Documentation
...
## Key Components
- Detailed breakdown of major modules/components
...
- Naming conventions
- Common patterns and practices used
