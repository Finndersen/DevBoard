...
class WorktreeSlot(Base):
    ...
    last_used_by_task: Mapped["Task | None"] = relationship(foreign_keys=[last_used_by_task_id])
    ...
    def get_current_branch(self) -> str | None:
        ...
        try:
            git = GitIntegration(self.path)
            # Run the async method synchronously
            return asyncio.run(git.get_current_branch())
        except Exception:
            # If we can't determine the branch (e.g., path doesn't exist), return None
            return None
