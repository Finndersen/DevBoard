...
class GitIntegration:
    ...
    async def validate(self) -> IntegrationConnectionResult:
        ...
        if not git_dir.exists():
            return IntegrationConnectionResult(success=False, message=f"Not a git repository: {self.codebase_path}")
        ...
        for line in output.split("\n"):
            ...
            if not line:
                if current_worktree:
                    ...
                    is_main = current_worktree.get("path") == str(self.codebase_path)
                    worktrees.append(
                        WorktreeInfo(
