    ...
    def run(self, args: CodexExecArgs) -> Iterable[str]:
        ...
        try:
            ...
            if returncode != 0:
                ...
                raise ThreadRunError(f"Codex Exec exited with code {returncode}: {stderr_text}")
        finally:
...
def _build_env(env_override: dict[str, str] | None, base_url: str | None, api_key: str | None) -> dict[str, str]:
...
def _codex_not_installed_error() -> CodexNotInstalledError:
    return CodexNotInstalledError(
        ...
        "See /Users/comfuture/Project/codex/README.md for full instructions."
    )
...
def _auth_required_error() -> AuthRequiredError:
    return AuthRequiredError(
        ...
        "via `api_key` or CODEX_API_KEY. See /Users/comfuture/Project/codex/README.md."
    )
